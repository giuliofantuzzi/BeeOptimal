...
if __name__ == '__main__':
    ...
    for function in BENCHMARK_FUNCTIONS:
        ...
        cost_history    = np.full((len(COLONY_SIZE),N_SIMULATIONS,MAX_ITERS+1),np.nan)

        for i,colony_size in enumerate(COLONY_SIZE):
            # Simulations
            for s in trange(N_SIMULATIONS,desc=f'Simulations (COLONY_SIZE={colony_size})'):
        ...
        for i,colony_size in enumerate(COLONY_SIZE):
            plt.plot(x,cost_medians[i,:],label=f"COLONY_SIZE={colony_size}",color=PLOT_COLORS[i])
